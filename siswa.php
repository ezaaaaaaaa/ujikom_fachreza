                ...
                <table class="table align-middle mb-0 bg-white">
                    ...
                    <tbody>
                        ...
                        if (isset($_GET['search'])) {
                            ...
                            $query = "SELECT * FROM siswa WHERE NIS LIKE '%$search%' OR nama LIKE '%$search%'";
                        } else {
                        ...
                        if (mysqli_num_rows($result) > 0) {
                            while ($row = mysqli_fetch_assoc($result)) {
                                ...
                                echo "<td>
                                        <a href='edit_siswa.php?id_siswa=" . htmlspecialchars($row['id_siswa']) . "' class='btn btn-warning btn-sm btn-rounded'>Edit</a>
                                        <a href='hapus_siswa.php?id_siswa=" . htmlspecialchars($row['id_siswa']) . "' onclick='return confirm(\"Yakin ingin menghapus data ini?\")' class='btn btn-danger btn-sm btn-rounded'>Hapus</a>
                                      </td>";
                                echo "</tr>";
                            }
                        } else {
